...
<script>
    ...
    export default {
        ...
        methods: {
            // 获取 easy-mock 的模拟数据
            getData() {
                axios.post(this.baseURL + '/user/search/' + this.currPage + '/' + this.pageSize, this.vo).then(res => {
            ...
            handleDelete(index, row) {
                ...
                this.$confirm('确定要删除吗？', '提示', {
                    ...
                    .then(() => {
                        axios.delete(this.baseURL + '/user/' + row.id).then(res => {
                            if (res.data.flag) {
                                this.getData();
            ...
            saveEdit() {
                ...
                if (this.form.id == null) {
                    result = axios.post(this.baseURL + '/user', this.form);
                } else {
                    result = axios.put(this.baseURL + '/user', this.form);
                }
                // 处理结果
                result.then(res => {
                    if (res.data.flag) {
